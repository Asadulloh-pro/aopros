...
class Aos {
    ...
    postData({ data, moduleName, stateName, path, loading, refresh = 'refresh', option = {}, }) {
        return __awaiter(this, void 0, void 0, function* () {
            const { setState } = this.useAllStore(moduleName);
            if (loading)
                setState({ [loading]: true });
            try {
                const result = yield this.services.create(path, data, option);
                if (stateName === 'drawer') {
                    setState({
                        [stateName]: {
                            isOpen: false,
                    ...
                    setState({
                        [stateName]: result === null || result === void 0 ? void 0 : result.data,
                    });
                }
                return result;
            }
    ...
    putData({ data, moduleName, stateName, path, loading, refresh = 'refresh', option = {}, }) {
        return __awaiter(this, void 0, void 0, function* () {
            const { setState } = this.useAllStore(moduleName);
            if (loading)
                setState({ [loading]: true });
            try {
                const result = yield this.services.update(path, data, option);
                if (stateName === 'drawer') {
                    setState({
                        [stateName]: {
                            isOpen: false,
